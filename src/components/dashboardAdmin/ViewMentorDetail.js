...
export default class ViewMentorDetail extends Component {
  ...
  componentDidMount() {
    window.scrollTo(0, 0);
    axios
      .get("/api/admin/allMentors/" + this.props.match.params.mentorid)
      .then((res) => {
  ...
  onVerifyClick = (e) => {
    e.preventDefault();
    axios
      .put("/api/admin/verifyMentor/" + this.props.match.params.mentorid)
      .then((res) => {
            ...
            {this.state.mentor.adminVerify ? (
              <div>
                ...
                <button
                  ...
                  className="btn btn-small waves-effect waves-light hoverable red"
                >
                  Suspend
                </button>
              </div>

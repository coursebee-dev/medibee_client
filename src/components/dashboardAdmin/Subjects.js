...
class Subjects extends Component {
    ...
    async AddSubCategory(e) {
        e.preventDefault()
        try {
            const { data } = await axios.post(`/api/admin/subject/addSubcategory/${e.target.value}`, {
                    ...
                    <div style={{ minWidth: "300px" }} className="card">
                        <div className="card-content">
                            <div className="row">
                                ...
                                <a className="btn secondary-content btn-small red modal-trigger" href="#subjectmodal">Add</a>
                                <div ref={Modal => {
                                    this.Modal = Modal;
                                }} id="subjectmodal" className="modal">
                                    <button className="secondary-content white btn-floating"><i className="secondary-content material-icons red-text modal-close">close</i></button>
                                    <div className="modal-content">
                                        <h4>Add a subject</h4>
                                        <div className="section">
                                            <form>
                                                <div className="row">
                                                    <div className="input-field col s12">
                                                        <input id="subject_name" type="text" className="validate" onChange={e => this.setState({ subject: e.target.value })} />
                                                        <label className="active" htmlFor="subject_name">Subject Name</label>
                                                    </div>
                                                    <div className="col s12">
                                                        <select className="browser-default" ref={Select => {
                                                            this.Select = Select;
                                                    ...
                                                    <div className="input-field col s12">
                                                        <button className="btn modal-close" onClick={this.AddSubject} >Add Subject</button>
                                                    </div>
                                                </div>
                                            </form>
                                        </div>
                                ...
                                {this.state.subjects.map(subjects => (
                                    <div key={subjects._id} className="card horizontal">
                                        ...
                                        <div className="card-stacked">
                                            <div className="card-content">
                                                <span className="card-title">{subjects.name} <span className="blue black-text badge">{subjects.category}</span></span>
                                            </div>
                                            <hr />
                                            <div className="card-content">
                                                {subjects.subcategory?.map(subcategories => (
                                                    <a className="chip blue" href="#!" key={subcategories._id}>
                                                        {subcategories.name}
                                                    </a>
                                                ))}
                                                <div className="row">
                                                    <div className="input-field col">
                                                        <input ref={subcat => this.subcat = subcat} id="subcategory_name" onChange={e => this.setState({ subcategory: e.target.value })} type="text" className="validate" />
                                                        <label className="active" htmlFor="subcategory_name">Subcategory Name</label>
                                                    </div>
                                                    <div className="input-field col">
                                                        <button value={subjects._id} onClick={this.AddSubCategory} className="btn btn-small red">Add subcategory</button>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
